...
Expected: a tray icon appears in the system tray. Right-click it, choose "Pairing & Settings" — a window opens showing a QR code and a token string in the read-only input. Enter a repo path (e.g. `C:\Users\Edy\source\Proiecte\Tool`) in the Repo Path field and click Save — the status text should show "Saved." Quit via the tray menu's "Quit" item and confirm the process exits.
